...
const Card = (props) => {
  ...
  return (
      <div
        ...
        {...listeners}
        style={style}
        className={`flex flex-col gap-3 w-80 rounded-lg border-t-12 cursor-grab ${props.status === "To-DO" ? "border-t-red-500" : props.status === "In Progress" ? "border-t-yellow-500" : "border-t-green-400"} border-2 border-gray-700 min-h-80 py-2 bg-green-50 px-2`}
      >
        ...
        <div className="p-3 rounded-md bg-amber-100">
          {props.status === "Done" ? (
            <p className="text-green-400 font-bold">Task is Completed</p>
          ) : new Date(props.date) < new Date() ? (
            <p className="text-red-500 font-bold">Task is Overdue!!</p>
          ) : (
            <p className="text-slate-700 font-bold">
              Time Left:{" "}
              {Math.ceil(
                (new Date(props.date) - new Date()) / (1000 * 60 * 60 * 24) + 1,
              )}{" "}
              days
            </p>
          )}
        ...
        <div className="flex justify-between font-bold text-lg px-2 rounded-md bg-blue-200 py-2 transition-all duration-300">
          <p
            onClick={handleDone}
            id="To-DO"
            className="text-blue-600 cursor-pointer hover:text-blue-500 hover:scale-105"
          >
            To DO
          </p>
          <p
            onClick={handleDone}
            id="In Progress"
            className="text-yellow-600 cursor-pointer hover:text-yellow-500 hover:scale-105"
          >
            In Progress
          </p>
          <p
            onClick={handleDone}
            id="Done"
            className="text-green-600 cursor-pointer hover:text-green-500 hover:scale-105"
          >
            Done
          </p>
        </div>
        <div className="flex justify-around font-bold rounded-md bg-amber-100 py-2 transition-all duration-300">
          <p
            ...
            className="flex gap-2 text-red-500 cursor-pointer hover:scale-105"
          >
            Delete <MdDelete size={25} />
          </p>
          <p
            onClick={() => props.editFunc(props.id)}
            className="flex gap-2 text-purple-500 cursor-pointer hover:scale-105"
          >
            Edit <FaEdit size={25} />
          </p>
        </div>
